...
if (count($this->urlData)==1){
    ...
    if($id==0){
            ...
            $arr_name=array('ID','Активность','Ответственный менеджер','Название','Адрес ФИАС','Контакты','Сайт','Граница закрытого периода','Дополнительная информация','Когда закрыт','Кто закрыл','Кто изменил','Когда изменил','Кто изменил границу закрытого прерида','Когда изменил границу закрытого периода', 'Адрес');
            ...
            $a=DBFetchNew($this->conn, "SELECT '', 'ввести адрес в ручную'  UNION SELECT id, address FROM ".$pref."fias");
    ...
    if($id>0){
        ...
        elseif(count($result)>0){
            ...
            $a=DBFetchNew($this->conn, "SELECT NULL,NULL as realname UNION SELECT id,realname FROM ".$pref."persons ORDER BY realname");
            $res['data_type']['id_person_close']['table']=array();
            foreach($a as $j => $b) $res['data_type']['id_person_close']['table']=$res['data_type']['id_person_close']['table']+array($b[0] => $b[1]);
