...
if (count($this->urlData)==1){
    ...
    if($id==0){
            ...
            $arr_name=array('Исполнитель','Специализация','Дополнительная информация');
            ...
            $a=DBFetchNew($this->conn, "SELECT id, name FROM ".$pref."speciality ORDER BY name");
    ...
    if($id>0){
        $query="SELECT * FROM ".$pref.$this->table_name." WHERE id_speciality=".$id." and id_worker='$id_worker'";
