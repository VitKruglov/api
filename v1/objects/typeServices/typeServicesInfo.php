...
//-------------------------------------------------------------//
//                                                             //
//        получение информации о типе услуге по договору по ID     //
//                                                             //
//-------------------------------------------------------------//
...
if (count($this->urlData)==1){
    ...
    if($id==0){
            ...
            $arr_name=array('ID','Активность','Когда изменил','Кто изменил','Название', 'Единица измерения', 'Дополнительная информация');
            ...
            $a=DBFetchNew($this->conn, "SELECT id,name_full FROM ".$pref."clients_type_units");
            $res['data_type']['id_type_units']['table']=array();
            foreach($a as $j => $b) $res['data_type']['id_type_units']['table']=$res['data_type']['id_type_units']['table']+array($b[0] => $b[1]);
